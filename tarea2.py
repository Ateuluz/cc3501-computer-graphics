...
# Set F1 Try #BUG
    # f1_1 = SceneElement(
    #     pipeline= flipper_pipeline,
    #     mesh_path="TemporaryHoldT2/my_assets/tri_f1car.obj",
    #     transform= tr.uniformScale(scale) @ tr.uniformScale(1/100)
    # )
...
# Set Special Functions
    ...
    def hit_main_mushroom(arbiter,space,data):
        print(f"Collision! {ps['score_add']} point{'s' if ps['score_add'] != 1 else ""} added!")
...
# Set Key Actions
    ...
    def on_key_press(symbol,modifiers):
        ...
        if symbol == key.A:
            ...
            print(">>",ball.matrix_draw())
        #     # BUG: Fisheye pipeline not well implemented
        #     # pipeline_next()
...
# Set Draw
    ...
    def on_draw():
        ...
        for fl in (flipper_izq,flipper_der,flipper_up):
            fl.transform_func = flipper_func
...
# Set Update World
    def update_world(dt, window):
        ...
        ranges = {
            flipper_izq: (-np.pi * 3 / 16, np.pi * 3 / 16,  1),
            flipper_der: ( np.pi * 3 / 16,-np.pi * 3 / 16, -1),
            flipper_up : (0              , np.pi / 2,       1),
        }
        for flipper, data in ranges.items():
            low, top, s = data
            if   (flipper.body.angle * s < low * s and flipper.body.angular_velocity * s < 0): 
                flipper.body.angular_velocity = 0
                flipper.body.angle            = low
            elif (flipper.body.angle * s > top * s and flipper.body.angular_velocity * s > 0):
                flipper.body.angular_velocity = 0
                flipper.body.angle            = top
